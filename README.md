...
- 保存内容：登录 Cookie、Overlay 弹幕样式、Overlay 窗口位置/大小
- 数据仅存本机，不收集遥测、不上传日志
...
| M3 | Overlay 窗口（透明/穿透/置顶/拖动/记忆位置） | ✅ |
| M4 | 弹幕渲染 + 样式配置 + 数量限制 | ✅ |
...
```text
src/                    Vue 前端
├── views/              主窗页面（房间 / 弹幕样式 / Overlay / 占位页）
├── overlay/            Overlay 弹幕渲染（独立入口）
└── components/         登录二维码弹窗等
